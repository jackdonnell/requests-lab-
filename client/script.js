...
sayHelloButton.addEventListener('mouseover', changeProperties)
...
const changeBackOriginal = () => {
    sayHelloButton.style.color ='black'
    sayHelloButton.style.backgroundColor = '#EFEFEF'
}
sayHelloButton.addEventListener('mouseout', changeBackOriginal)
...
const ohMy = () => {
    ...
    .then(res => {
        ...
        for (let i = 0; i < res.data.length; i ++) {
            let p = document.createElement('p').textContent = res.data[i]
            p.appendChild('p')
        }
    })
    .catch(error => {
